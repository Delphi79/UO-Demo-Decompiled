...
TRIGGER( speech , "*I resign from my guild*" )(obj speaker, string arg)
{
  if(speaker != this)
  {
    return(0x01);
...
{
  loc Q55H = loc( 0x00, 0x00, (0x00 - 0x50) );
...
TRIGGER( message , "guildGone" )(obj sender, list args)
{
...
TRIGGER( textentry , 0x21 )(obj sender, int button, string text)
{
  if(sender != this)
  {
    return(0x00);
  ...
  if(button == 0x01)
  {
    ...
    if(text == Q5CA)
    {
      ...
      if(Q48Z > 0x00)
      {
        ...
        args = list( player, Q48Z, 0x00, Q5IU );
...
TRIGGER( message , "refreshAggression" )(obj sender, list args)
{
  refreshAggression(this);
  return(0x00);
}

TRIGGER( message , "refreshCriminal" )(obj sender, list args)
{
  loc Q4F5 = loc( args[0x00] );
  committedCrimeAt(this, Q4F5, 0x01E0)/* semicolon added by the decompiler post-processor */;
  return(0x00);
}

TRIGGER( message , "changeReputation" )(obj sender, list args)
{
...
TRIGGER( message , "murderReport" )(obj sender, list args)
{
  int Q67H = args[0x00];
  ...
  if(murderCount >= 0x05)
  {
    ...
    args = list( this, murderCount, getSex(this), Q4UT, Q4UR, getHue(this) );
...
TRIGGER( message , "bountyInfo" )(obj sender, list args)
{
  obj Q5IS = args[0x00];
