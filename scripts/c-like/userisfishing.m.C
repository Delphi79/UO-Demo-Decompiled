...
TRIGGER( message , "fish" )(obj sender, list args)
{
  obj pole = args[0x00];
  loc there = loc( args[0x01] );
...
TRIGGER( callback , 0x4B )()
{
  ...
  {
    there = loc( getObjVar(this, "there") );
  }
...
FUNCTION void Q5NI()
{
  ...
  {
    location = loc( getObjVar(pole, "location") );
