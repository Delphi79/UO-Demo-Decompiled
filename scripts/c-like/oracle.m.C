...
TRIGGER( textentry , 0x22 )(obj sender, int button, string text)
{
  Q4FN = text;
...
TRIGGER( textentry , 0x24 )(obj sender, int button, string text)
{
  list u = list( text, Q4FN );
  addMessage(u);
  return(0x00);
}

TRIGGER( textentry , 0x23 )(obj sender, int button, string text)
{
  int Q4Y2 = Q4PC(text);
  if(Q4Y2 == (0x00 - 0x01))
  {
...
TRIGGER( typeselected , 0x3A )(obj user, int listindex, int objtype, int objhue)
{
  if(listindex == 0x00)
  {
    return(0x00);
