...
FUNCTION int Q41R(obj user, obj usedon)
{
  ...
  if(canSeeObj(user, usedon) == 0x01)
  {
    if(isPlayer(usedon))
    {
      ...
      loc target = loc( getLocation(usedon) );
...
{
  loc Q5HP;
  list Q5HN = list( 0x0FE6, 0x0FE7, 0x0FE8, 0x0FE9, 0x0FEA, 0x0FEB, 0x0FEC, 0x0FED, 0x0FEE );
  loc blah = loc( where );
  ...
  list Q5HO = list( 0x120E, 0x120F, 0x1210, 0x1211, 0x1212, 0x1213, 0x1214, 0x1215, 0x1216 );
...
{
  list Q4AB = list( 0x0A26, 0x0A27, 0x0A28, 0x0A29, 0x142F, 0x1433, 0x1437, 0x1853, 0x1857, 0x1C16 );
  list Q514;
  int count = 0x00;
